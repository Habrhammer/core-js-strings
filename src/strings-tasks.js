...
function repeatString(str, times) {
  return times > 0 && str && str.repeat(times);
}
...
function sumOfCodes(str) {
  if (!str) return 0;
  return [...str].reduce((acc, prev) => {
    return acc + prev.charCodeAt();
  }, 0);
}
...
function isPalindrome(str) {
  return (
    ...
    [...str].reverse().join``.toLowerCase().replace(/[^a-zA-Z0-9]/g, '')
  );
}
...
function findLongestWord(sentence) {
  const maxLength = Math.max(...sentence.split(' ').map((e) => e.length));
  return sentence.split(' ').find((word) => word.length === maxLength);
}
...
function encodeToRot13(str) {
  const charToRot13 = (char) => {
    const origAlphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    const rot13Alphabet =
      'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    const index = origAlphabet.indexOf(char);
    return index === -1 ? char : rot13Alphabet[index];
  };
  return [...str].map((currentChar) => charToRot13(currentChar)).join``;
}
